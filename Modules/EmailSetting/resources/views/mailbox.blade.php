...
@section('title')<title>{{ __('Mailbox') }}</title>@endsection

@section('body-header')
    <h3 class="crancy-header__title m-0">{{ __('Mailbox') }}</h3>
    <p class="crancy-header__text">{{ __('Email') }} >> {{ __('Mailbox') }}</p>
@endsection
...
.mailbox-filter-btns { display:flex; gap:6px; }
.mailbox-filter-btn { padding:7px 14px; border-radius:6px; border:1px solid #e8eaf0; background:#fff; font-size:12px; color:#4a4f73; cursor:pointer; transition:all .15s; }
                ...
                <div class="mailbox-wrap">
                    ...
                    <div class="mailbox-sidebar">
                        <div class="mailbox-sidebar-inner">
                            ...
                            {{-- Mail Folders --}}
                            <p class="mailbox-section-title">{{ __('Mailbox') }}</p>

                            <a href="{{ route('admin.mailbox.index') }}" class="mailbox-folder-item {{ !request('status') && !request('account') ? 'active' : '' }}">
                                <i class="fas fa-paper-plane"></i>
                                <span>{{ __('All Sent') }}</span>
                                <span class="folder-count ml-auto">{{ $stats['total'] }}</span>
                            </a>
                            ...
                            @forelse($accounts as $i => $account)
                                <a href="{{ route('admin.mailbox.index', ['account' => $account->id]) }}"
                                   class="mailbox-account-item {{ request('account') == $account->id ? 'active' : '' }}">
                    ...
                    <div class="mailbox-main">
                        ...
                        <div class="mailbox-toolbar">
                            <div class="mailbox-search">
                                ...
                                <form method="GET" id="searchForm">
                                    ...
                                    <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search subject, to, from...') }}"
                        ...
                        <div class="mailbox-list" id="mailList">
                            @forelse($logs as $log)
                            <div class="mailbox-item" id="mail-row-{{ $log->id }}" onclick="openMail({{ $log->id }})">
...
function openMail(id) {
    ...
    fetch('{{ route("admin.mailbox.index") }}/' + id, {
    ...
    .then(function(data) {
        ...
        document.getElementById('detailMeta').innerHTML =
            '<div class="mb-1"><span>{{ __("From:") }}</span> ' + escHtml(data.from) + '</div>' +
            '<div class="mb-1"><span>{{ __("To:") }}</span> ' + escHtml(data.to) + '</div>' +
            (data.cc ? '<div class="mb-1"><span>{{ __("CC:") }}</span> ' + escHtml(data.cc) + '</div>' : '') +
            (data.account ? '<div class="mb-1"><span>{{ __("Account:") }}</span> ' + escHtml(data.account) + '</div>' : '') +
            '<div class="mb-1"><span>{{ __("Date:") }}</span> ' + (data.sent_at || '—') + '</div>' +
            '<div>' + statusBadge + '</div>';
        ...
        // Wire up delete form
        document.getElementById('deleteMailForm').action = '{{ url("admin/mailbox") }}/' + id;
    })
    .catch(function() {
