        ...
        <div class="grid xl:grid-cols-2 grid-cols-1 items-center xl:gap-16 gap-12">
            ...
            <div class="xl:pb-24 relative z-10" data-aos="fade-right">
                ...
                <div class="inline-flex items-center gap-2.5 bg-white border border-purple/15 rounded-full px-5 py-2.5 mb-6 shadow-sm">
                    ...
                    <span class="text-purple text-sm font-semibold tracking-wide">{{ getTranslatedValue($cc_hero, 'badge_text') ?: 'Creative Content Studio' }}</span>
                </div>
                ...
                <h1 class="text-4xl md:text-65 text-main-black mb-[35px] pointer-events-auto custom-heading md:text-left" style="font-weight: 400 !important;">
                    {!! getTranslatedValue($cc_hero, 'heading') ?: 'We Create <span>Creative</span> Content That Stands Out' !!}
                </h1>

                <div class="px-6 py-[14px] bg-white border-l-2 border-blue-sass mb-[35px] xl:w-full md:w-[500px]">
                    <p class="text-ptwo text-paragraph">
                        {{ getTranslatedValue($cc_hero, 'description') ?: 'We help startups, local businesses, and brands build a strong online presence. High-quality reels, graphics, motion visuals, and brand content crafted to capture attention and build engagement.' }}
                    </p>
                </div>

                <div class="flex space-x-[30px] items-center pointer-events-auto">
                    <a href="{{ getTranslatedValue($cc_hero, 'cta_button_1_url') ?: route('contact-us') }}">
                        <div class="home-two-btn-bg py-3 group bg-purple border-purple">
                            <span class="text-base text-white group-hover:text-purple transition-all duration-300 font-semibold font-inter relative z-10">
                                {{ getTranslatedValue($cc_hero, 'cta_button_1') ?: 'Book Your Meeting' }}
                            </span>
                            <span>{{ get_svg('home_cta_white') }}</span>
                        </div>
                    </a>
                    <a href="{{ getTranslatedValue($cc_hero, 'cta_button_2_url') ?: route('services') }}">
                        <div class="flex items-center gap-2 group">
                            <p class="mb-[1px] font-medium text-main-black leading-5 font-inter border-b border-main-black before:block before:pb-[1px] before:border-purple before:font-medium before:text-purple before:leading-5 before:font-inter before:border-b relative overflow-hidden before:content-[''] before:absolute before:-bottom-[1px] before:transition-all before:duration-300 before:w-0 hover:before:w-full before:h-[21px]">
                                {{ getTranslatedValue($cc_hero, 'cta_button_2') ?: 'View Our Work' }}
                            </p>
                ...
                <div class="flex items-center gap-4 mt-10">
                    ...
                    <div>
                        <p class="text-xs font-bold text-main-black">{{ getTranslatedValue($cc_hero, 'trust_text') ?: '50+ Happy Brands' }}</p>
                    ...
                    <span class="text-xs text-paragraph font-medium pl-2 border-l border-gray-200">{{ getTranslatedValue($cc_hero, 'trust_subtext') ?: 'Trusted by growing brands across India' }}</span>
                </div>
            </div>
            ...
            <div class="relative flex justify-center xl:justify-end items-center" data-aos="fade-left">
                ...
                <div class="cc-stat-badge pop-card pop-d1">
                    <div class="flex items-center gap-2 mb-1.5">
                        ...
                        <p class="text-xs text-paragraph font-medium">{{ getTranslatedValue($cc_hero, 'stat_1_label') ?: 'Total Reach' }}</p>
                    </div>
                    <p class="text-xl font-bold text-main-black">{{ getTranslatedValue($cc_hero, 'stat_1_value') ?: '1.26M' }}</p>
                ...
                <div class="cc-stat-badge pop-card pop-d2">
                    <div class="flex items-center gap-1.5 mb-1">
                        ...
                        <p class="text-xs text-paragraph font-medium">{{ getTranslatedValue($cc_hero, 'stat_2_label') ?: 'Growth Rate' }}</p>
                    </div>
                    <p class="text-xl font-bold text-main-black">{{ getTranslatedValue($cc_hero, 'stat_2_value') ?: '+34.12%' }}</p>
                ...
                <div class="absolute left-1/2 -translate-x-1/2 xl:bottom-4 bottom-2 z-20 cc-float">
                    <div class="bg-purple text-white px-5 py-2.5 rounded-full shadow-purple flex items-center gap-2 pop-card pop-d3">
                        <p class="text-sm font-bold">{{ getTranslatedValue($cc_hero, 'stat_3_value') ?: '84.2%' }}</p>
                        <p class="text-[10px] opacity-80">{{ getTranslatedValue($cc_hero, 'stat_3_label') ?: 'Engagement Rate' }}</p>
                    </div>
                </div>

                {{-- Floating card: stat 4 --}}
                ...
                <div class="cc-stat-badge pop-card pop-d4">
                    <div class="flex items-center gap-1.5 mb-1">
                        ...
                        <p class="text-xs text-paragraph font-medium">{{ getTranslatedValue($cc_hero, 'stat_4_label') ?: 'Followers' }}</p>
                    </div>
                    <p class="text-xl font-bold text-main-black">{{ getTranslatedValue($cc_hero, 'stat_4_value') ?: '48.6k' }}</p>
                </div>
                </div>
