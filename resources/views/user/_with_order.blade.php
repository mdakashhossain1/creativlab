<div class="flex flex-wrap justify-between sm:items-center gap-30 mb-5">
    ...
    <form class="dashbord_table_top_right" method="GET" action="{{ route('user.orders') }}">
        ...
        <tbody class="divide-y divide-[#E0E1E1]">
            ...
            <tr class="bg-white transition-all">
                <td class="px-30 py-3.5 text-start">
                    <div>
                        <a href="{{ route('user.order_show', $order->order_id) }}" class="text-16 text-[#4A7DFF] hover:underline">{{ $order->order_id }}</a>
                    </div>
                </td>
                <td class="px-30 py-3.5 text-start">
                    <div>
                ...
                <td class="px-30 py-3.5 text-start">
                    @if ($order->order_status == 0)
                        <div>
                            <p class="text-16 text-nowrap status status-pending">{{ __('Pending') }}
                            </p>
                        </div>
                    @endif
                    @if ($order->order_status == 2)
                        <div>
                            <p class="text-16 text-nowrap status status-pending">{{ __('Rejected') }}
                            </p>
                        </div>
                    @endif
                    @if ($order->order_status == 3)
                        <div>
                            <p class="text-16 text-nowrap status status-success">
                                {{ __('Processing') }}</p>
                        </div>
                    @endif
                    @if ($order->order_status == 4)
                        <div>
                            <p class="text-16 text-nowrap status status-success">{{ __('Shipped') }}
                            </p>
                        </div>
                    @endif
