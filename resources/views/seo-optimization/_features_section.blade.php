        ...
        <div class="grid xl:grid-cols-2 grid-cols-1 gap-12 xl:gap-16 items-center">
            ...
            <div data-aos="fade-left">
                <span class="inline-flex items-center gap-2 text-purple text-xs font-bold uppercase tracking-[0.2em] bg-[#EDE8FF] px-4 py-2 rounded-full mb-5">
                    ...
                    {{ getTranslatedValue($seo_features, 'badge_text') ?: 'SEO Solutions' }}
                </span>

                <h2 class="xl:text-[44px] md:text-[36px] text-[26px] font-bold text-main-black leading-[1.12] mb-5">
                    {!! getTranslatedValue($seo_features, 'heading') ?: 'Boost Rankings.<br>Increase Traffic.<br><span class="text-purple">Grow Your Business.</span>' !!}
                </h2>

                <p class="text-paragraph text-base leading-7 mb-8 xl:max-w-[460px]">
                    {{ getTranslatedValue($seo_features, 'description') ?: 'Our data-driven SEO strategies help your website rank higher, attract qualified visitors, and deliver the right audience consistently.' }}
                </p>

                @php
                    $seoFeatDefaults = ['Highest search rankings','Quality organic traffic','Better user experience','Long-term sustainable growth'];
                @endphp
                <div class="grid sm:grid-cols-2 gap-4 mb-9">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <p class="font-semibold text-main-black text-sm">{{ getTranslatedValue($seo_features, "feature_{$i}") ?: $seoFeatDefaults[$i - 1] }}</p>
                    </div>
                    @endfor
                </div>

                <a href="{{ getTranslatedValue($seo_features, 'cta_button_url') ?: route('contact-us') }}"
                   class="inline-flex items-center gap-2.5 bg-purple text-white font-bold text-sm uppercase tracking-wider px-8 py-4 rounded-full hover:bg-main-black transition-all duration-300">
                    {{ getTranslatedValue($seo_features, 'cta_button') ?: 'Get Free SEO Audit' }}
