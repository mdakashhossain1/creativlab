<style>
    ...
    @media (max-width: 1024px) { .wd-cta-img { width: 460px; height: 460px; } }
    @media (max-width: 767px) { .wd-cta-img-wrap { width: 100%; } .wd-cta-img { width: 100%; height: auto; max-width: 420px; } .wd-cta-card { display: none !important; } }
    @media (max-width: 480px) { .wd-cta-img { max-width: 300px; } }
</style>
        ...
        <div class="relative w-full rounded-[24px] overflow-hidden" style="background: linear-gradient(120deg, #EDE8FF 0%, #F4F1FF 50%, #E4DCFF 100%);">
            ...
            <div class="relative z-10 flex flex-col md:flex-row items-center justify-between px-8 md:px-16 xl:px-20 py-14 md:py-0 md:min-h-[380px]">
                ...
                <div class="md:max-w-[480px] w-full md:py-16 text-center md:text-left" data-aos="fade-right">
                    <span class="text-purple text-xs font-bold uppercase tracking-[0.2em] mb-4 block">{{ getTranslatedValue($wd_cta, 'section_label') ?: 'GET STARTED TODAY' }}</span>
                    <h2 class="xl:text-[48px] md:text-[38px] text-[28px] font-bold text-main-black leading-tight mb-4">
                        {!! getTranslatedValue($wd_cta, 'heading') ?: 'Ready to Build<br>Your <span class="text-purple">Online Presence?</span>' !!}
                    </h2>
                    <p class="text-paragraph text-base leading-7 mb-8">
                        {{ getTranslatedValue($wd_cta, 'description') ?: "Let's create a website that represents your brand and helps your business grow digitally." }}
                    </p>
                    <div class="flex flex-wrap gap-4 md:justify-start justify-center">
                        <a href="{{ getTranslatedValue($wd_cta, 'button_1_url') ?: route('contact-us') }}"
                           class="inline-flex items-center gap-2.5 bg-purple text-white font-bold text-sm uppercase tracking-wider px-8 py-4 rounded-full hover:bg-main-black transition-all duration-300 shadow-purple">
                            {{ getTranslatedValue($wd_cta, 'button_1_text') ?: 'Build Your Website' }}
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M5 12H19M19 12L13 6M19 12L13 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </a>
                        <a href="{{ getTranslatedValue($wd_cta, 'button_2_url') ?: route('services') }}"
                           class="inline-flex items-center gap-2 text-main-black font-semibold text-sm hover:text-purple transition-colors duration-300 px-2">
                            {{ getTranslatedValue($wd_cta, 'button_2_text') ?: 'View Services' }}
                ...
                <div class="wd-cta-img-wrap relative md:self-center self-center mt-8 md:mt-0 flex-shrink-0 flex items-center justify-center" data-aos="fade-left">
                    <img src="{{ asset('frontend/assets/images/web-delopmnet/bottom-image.webp') }}"
                         alt="Web Development" class="wd-cta-img" style="object-fit:contain; object-position:center; display:block;">
                    ...
                    <div class="wd-cta-card wd-float-b absolute left-0 top-16 z-20 bg-white rounded-2xl px-4 py-3 flex items-center gap-3"
                        ...
                        <div>
                            <p style="font-size:9px; color:#6B7280; margin:0; line-height:1.2;">{{ getTranslatedValue($wd_cta, 'card_1_label') ?: 'Web Presence' }}</p>
                            <p style="font-size:12px; font-weight:700; color:#111827; margin:0; white-space:nowrap;">{{ getTranslatedValue($wd_cta, 'card_1_value') ?: 'Live & Online' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="wd-cta-card wd-float-a absolute right-0 bottom-20 z-20 bg-white rounded-2xl px-4 py-3 flex items-center gap-3"
                        ...
                        <div>
                            <p style="font-size:9px; color:#6B7280; margin:0; line-height:1.2;">{{ getTranslatedValue($wd_cta, 'card_2_label') ?: 'Clean Code' }}</p>
                            <p style="font-size:12px; font-weight:700; color:#111827; margin:0; white-space:nowrap;">{{ getTranslatedValue($wd_cta, 'card_2_value') ?: 'Built to Scale' }}</p>
                        </div>
                    </div>
                </div>
            </div>
